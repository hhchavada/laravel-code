    ...
    public function up(): void
    {
        if (!Schema::hasTable('blog_analytics')) {
            Schema::create('blog_analytics', function (Blueprint $table) {
                $table->id();
                $table->enum('type', ['view','share','poll_share','blog_time_spent','app_time_spent','blog_poll_option'])->default('view');
                $table->integer('user_id')->default(0);
                $table->string('player_id', 255)->nullable();
                $table->string('fcm_token', 255)->nullable();
                $table->unsignedBigInteger('blog_id')->default(0);
                $table->unsignedBigInteger('blog_poll_option_id')->default(0);
