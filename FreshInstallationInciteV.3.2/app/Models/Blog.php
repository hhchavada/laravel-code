...
class Blog extends Model
{
    ...
    // Define the visibilities relationship
    public function visibilities()
    {
        return $this->hasMany(BlogVisibility::class, 'blog_id');
    }

    // Define the questions relationship
    public function questions()
    {
        return $this->hasMany(BlogQuestion::class, 'blog_id');
    }
                ...
                if($entry_id){
                    if(isset($image) && $image!=''){
                        foreach($image as $image_data){                            
                            $uploadImage = \Helpers::uploadFilesAfterResizeCompressOriginalName($image_data,'blog');
            ...
            else
            {
                ...
                if(isset($data['button_name']) && $data['button_name']!=''){
                    ...
                    if($button_name=='Submit'){
                        ...
                        if(setting('enable_send_push_notification_on_post_published') == 1){
                            $model = new Blog();
                            $model->sendNotification($id);
                        }
                    }
                    unset($data['button_name']);
                }else{
                ...
                if (!empty($removedImages)) {
                    foreach ($removedImages as $imageName) {
                        ...
                        $blogImage = BlogImage::where('image', $imageName)->first();
                        if ($blogImage) {
                            $blogImage->delete();
                        }
                    }
    	...
    	try {
            ...
            {     
                ...
                if($entry_id){
                    if(isset($image) && $image!=''){
                        foreach($image as $image_data){                            
                            $uploadImage = \Helpers::uploadFilesAfterResizeCompressOriginalNameQuote($image_data,'blog');
                            if($uploadImage['status']==true){
                                $postArr = array(
                                    'blog_id'=> $entry_id,
                                    'image'=> $uploadImage['file_name'],
    ...
    **/
    ...
    public static function sendNotification($blogId)
    { 
        $id = $blogId;
        if(setting('enable_os_notifications') == 1){
           if(setting('one_signal_key')==''){
            return '';
                ...
                $blog = Blog::select('id', 'type', 'title','description', 'source_name', 'source_link','voice', 'accent_code','video_url', 'is_voting_enable', 'schedule_date','created_at', 'updated_at', 'background_image')->where('id',$id)->first();
