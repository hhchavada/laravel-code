...
class SettingAPIController extends Controller
{
    ...
    function getSettings(Request $request)
    {
        try {
            ...
            if ($data->count()) {
                foreach ($data as $row) {
                    $finalData[$row->key] = $row->value === "" ? null : $row->value;
                }
            }
    ...
    function getCms(Request $request)
    {
        try {
            ...
            // Fetch CMS content with translation for selected language
            $data = CmsContent::with(['translations' => function($query) {
                $query->where('language_code', $this->language);
            }])->get();
    
            // Format the response with translations if they exist
            $result = $data->map(function ($row) {
                $translation = $row->translations->first();
                return [
                    'id' => $row->id,
                    'image' => url('uploads/cms/' . $row->image),
                    'title' => $translation ? $translation->title : null,
                    'description' => $translation ? $translation->description : null,
                ];
            });
    
            return $this->sendResponse($result, __('lang.message_data_retrived_successfully'));
