                                ...
                                <form class="add-new-record pt-0 row g-2" id="edit-record_{{$row->id}}" action="{{url('admin/update-sub-admin')}}" method="POST" enctype="multipart/form-data" > @csrf 
                                    ...
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                            ...
                                            <input type="text" class="form-control" id="name" placeholder="{{__('lang.admin_name_placeholder')}}" name="name" value="{{$row->name}}" required>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                            <label class="form-label" for="email">{{__('lang.admin_email')}} <span class="required">*</span></label>
                                            <input type="email" class="form-control" id="email" placeholder="{{__('lang.admin_email_placeholder')}}" name="email" value="{{$row->email}}" required>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="d-flex-p">
                                            <img src="{{ asset('uploads/user')}}/{{$row->photo}}" id="image-preview_{{$row->id}}" class="rounded me-50 image-preview-cls" alt="profile image" height="80" width="80" onerror="this.onerror=null;this.src=`{{ asset('uploads/image_preview.png') }}`" />
                                            <div class="img-footer">
                                            <label class="btn btn-primary me-75 mb-0" for="change-picture_{{$row->id}}">
                                                <span class="d-none d-sm-block">@if($row->photo == '') {{__('lang.admin_upload')}} @else {{__('lang.admin_image_replace')}} @endif</span>
                                                <input class="form-control" type="file" id="change-picture_{{$row->id}}" hidden accept="image/png, image/jpeg, image/jpg" name="photo" onclick="showImagePreview('change-picture_{{$row->id}}','image-preview_{{$row->id}}',512,512);"/>
