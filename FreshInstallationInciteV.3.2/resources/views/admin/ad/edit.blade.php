...
<div class="container-xxl flex-grow-1 container-p-y">
    <form id="add-record" onsubmit="return validateEditAds('edit-record');" action="{{url('admin/update-ad')}}" method="POST" enctype="multipart/form-data">
    @csrf
                ...
                <div class="tab-content">
                    <div class="tab-pane fade active show" id="form-tabs-personal" role="tabpanel">
                        ...
                        <div class="col-md-12" style="margin-top: 15px;">
                            ...
                            <div class="col mt-2">
                                <div class="form-check form-check-inline">
                                    ...
                                    value="image"
                                    id="image"
                                    onclick="selectMediaTypeRadio('image')"
                                ...
                                <div class="form-check form-check-inline">
                                    ...
                                    type="radio"
                                    id="video_url"
                                    onclick="selectMediaTypeRadio('video_url')"
                                    @if($row->media_type=='video_url') checked="" @endif
                                    />
                                ...
                                <div class="form-check form-check-inline">
                                    ...
                                    value="video"
                                    id="video"
                                    onclick="selectMediaTypeRadio('video')"
                        ...
                        <div class="col-md-12 showImage @if($row->media_type=='video' || $row->media_type=='video_url') hide @endif" style="margin-top: 15px;">
                            ...
                            <img src="{{url('uploads/ad/'.$row->media)}}" class="rounded me-50" id="image-preview" alt="image" height="80" width="80" style="margin-top: 20px;" onerror="this.onerror=null;this.src=`{{ asset('uploads/no-image.png') }}`"/>
                        </div>
                        <div class="col-md-12 @if($row->media_type=='image' || $row->media_type=='video_url') hide @endif showVideo" style="margin-top: 15px;">
                            ...
                            <video id="video-preview" controls="false" style="width: 30%;margin-top: 20px;" class="video-container @if($row->media=='') hide @endif">
                                <source class="video-preview" id="video-preview" src="{{ $row->media !='' ? url('uploads/ad/video/'.$row->media) : '';}}" type="video/mp4" controls="false">
                            </video>
                        </div>
                        <div class="col-md-6 @if($row->media_type=='video' || $row->media_type=='image') hide @endif showVideoUrl" style="margin-top: 15px;">
                            ...
                            <input value="{{$row->video_url}}" class="form-control video_url_val" type="text" name="video_url" id="video_url" placeholder="{{__('lang.admin_video_url_placeholder')}}">
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    .then(editor => {
     ...
     function validateEditAds(formid) {
        var $form = $("#" + formid);
        var data = new FormData($form[0]);
        var mediaType = getSelectedMediaType();
        var existingImageSrc = document.getElementById('image-preview').src;
        var existingVideoSrc = document.getElementById('video-preview').src;
        var video_url_val = $(".video_url_val").val();
        if (data.get('title') == '') {
            ...
            if(mediaType=='image'){
                ...
                if(video_url_val==''){
                    myToastr(adminTranslation.admin_video_url_error, 'error');
                    return false;
                }else{
                    return true;
                }
    ...
    function getSelectedMediaType() {
        var selectedValue = $('input[name="media_type"]:checked').val();
        return selectedValue;
    }
    
    
    function selectMediaTypeRadio(type) {
        if(type=='image'){
