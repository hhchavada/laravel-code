...
class RssFeedItemController extends Controller
{
    ...
    { 
        ...
        try{
            ...
            if(isset($request) && $request->category_id != '' || $request->source_id !=''){
                
            $data['result'] = RssFeed::getFeedLists($request);
            if(isset($data['result']['status']) && $data['result']['status']== false){
                
               $data['result'] = RssFeed::getFeedListsWhenError($request);
               $data['categories'] = Category::where('status',1)->where('parent_id',0)->with('sub_category')->get();
    ...
    public function store(Request $request)
    {
        ...
        if($blog_id){
                ...
                if($category){                    
                    ...
                    if($category->parent_id==0){
                        ...
                        if($subcategory){
                            ...
                            BlogCategory::insert($image_cat_arr);
                        }
            ...
            return redirect()->back()->with('success', __('lang.message_data_retrived_successfully')); 
        }        
    }
    /**
    ...
    **/
    ...
    public function getSourcesByCategory(Request $request)
    {
        $categoryId = $request->input('category_id');
        $sources = RssFeed::where('category_id', $categoryId)->where('status', 1)->get();
