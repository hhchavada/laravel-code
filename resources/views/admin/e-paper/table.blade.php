<table class="table">
    <thead class="table-light">
        <tr class="text-nowrap">
            ...
            <th>{{__('lang.admin_share_count')}}</th>
                ...
                <tr>
                    ...
                    <td>
                        @if($row->pdf)
                            ...
                            <a target="_blank"
                               class="rounded me-50"
                               href="{{ asset('uploads/e-paper/pdf/' . $row->pdf) }}">
                                <i class="ti ti-eye me-1 margin-top-negative-4"></i>
                            </a>

                            {{-- DOWNLOAD PDF --}}
                            <a class="rounded me-50"
                               href="{{ route('admin.epaper.download', $row->id) }}">
                                <i class="ti ti-download me-1 margin-top-negative-4"></i>
                            </a>
                        @else
                            --
                        @endif
                    ...
                    <td>
                        ...
                        <div class="offcanvas offcanvas-end" id="edit-new-record_{{$row->id}}">
                            ...
                            <div class="offcanvas-body flex-grow-1">
                                <form onsubmit="return validateEditEpaper('edit-record-{{$row->id}}');" class="add-new-record pt-0 row g-2" id="edit-record" action="{{url('admin/update-e-paper')}}" method="POST" enctype="multipart/form-data"> @csrf 
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                            ...
                                            <div class="d-flex-p">
                                                @if($row->pdf!='')
                                                 <iframe src="{{ asset('uploads/e-paper/pdf')}}/{{$row->pdf}}" class="rounded me-50 mb-2 image-preview-cls" id="edit-pdf-preview_{{$row->id}}" alt="image" height="80" width="80"></iframe>
                                                @else
                                                 <iframe src="" class="rounded me-50 mb-2 hide image-preview-cls" id="edit-pdf-preview_{{$row->id}}" alt="image" height="80" width="80"></iframe>
                                                @endif
                                                <div class="img-footer">
                                                    <label class="btn btn-primary me-75 mb-0" for="change-pdf_{{$row->id}}">
                                                    <span class="d-none d-sm-block">{{__('lang.admin_image_replace')}}</span>
                                                    <input class="form-control" type="file" name="pdf" id="change-pdf_{{$row->id}}" hidden accept=".pdf" name="pdf" onchange="showPdfPreview(this, 'edit-pdf-preview_{{$row->id}}');"/>
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="d-flex-p">
                                            <img src="{{ asset('uploads/e-paper')}}/{{$row->image}}" id="image-preview_{{$row->id}}" class="rounded me-50 image-preview-cls" alt="profile image" height="80" width="80" onerror="this.onerror=null;this.src=`{{ asset('upload/image_preview.png') }}`" />
                                            <div class="img-footer">
