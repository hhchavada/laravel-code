        ...
        @if(count($result) > 0) 
            @foreach($result as $row) 
                ...
                <tr>
                    ...
                    <td>
                        @if (Gate::check('update-category'))
                            <a href="{{url('admin/post?category_id='.$row->id)}}">{{$row->blog_count}}</a>
                        @else
                            {{$row->blog_count}}
                        @endif
                    </td>
                    ...
                    <td>
                        ...
                        <div class="offcanvas offcanvas-end" id="edit-new-record_{{$row->id}}">
                            ...
                            <div class="offcanvas-body flex-grow-1">
                                <form class="add-new-record pt-0 row g-2" id="edit-record" action="{{url('admin/update-category')}}" method="POST" enctype="multipart/form-data" onsubmit="return validateCategory('edit-new-record_{{$row->id}}');"> @csrf 
                                    @if($row->parent_id!=0)
                                    <div class="col-sm-12">
                                        <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="d-flex-p">
                                            <img src="{{ asset('uploads/category')}}/{{$row->image}}" id="image-preview_{{$row->id}}" class="rounded me-50 image-preview-cls" alt="profile image" height="80" width="80" onerror="this.onerror=null;this.src=`{{ asset('uploads/image_preview.jpg') }}`" />
