...
<div class="container-xxl flex-grow-1 container-p-y">
  <h4 class="fw-bold py-3 mb-4 display-inline-block">
    <span class="text-muted fw-light">
      <a href="{{url('admin/dashboard')}}">{{__('lang.admin_dashboard')}}</a> / </span> {{__('lang.admin_live_news')}} {{__('lang.admin_list')}}
  </h4>
  @can('add-live-news')
  <button class="btn btn-secondary btn-primary float-right mt-3" type="button" data-bs-toggle="offcanvas" data-bs-target="#add-new-record" aria-controls="add-new-record">
    <span>
      <span class="d-md-inline-block d-none">{{__('lang.admin_create_live_news')}}</span>
    </span>
  </button>
  @endcan
      ...
      <form method="get">
        <div class="row">
          ...
          <div class="form-group col-sm-3 display-inline-block" >
              <input type="text" class="form-control" placeholder="{{__('lang.admin_company_name')}}" name="company_name" value="@if(isset($_GET['company_name']) && $_GET['company_name']!=''){{$_GET['company_name']}}@endif">
          </div>
          ...
          <div class="col-sm-3 display-inline-block">
            ...
            <a type="reset" class="btn btn-outline-secondary" href="{{url('admin/live-news')}}">{{__('lang.admin_reset')}}  </a>
          </div>
        </div>
      </form>
    </div>
  ...
  <div class="card">
    <div class="card-header">
      <h5 class="card-title display-inline-block">{{__('lang.admin_live_news')}} {{__('lang.admin_list')}}</h5>
    ...
    <div class="table-responsive text-nowrap"> @include('admin/live-news/table') </div>
  ...
  <div class="offcanvas offcanvas-end" id="add-new-record">
    <div class="offcanvas-header border-bottom">
      <h5 class="offcanvas-title" id="exampleModalLabel">{{__('lang.admin_add_live_news')}}</h5>
      <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body flex-grow-1">
      <form class="add-new-record pt-0 row g-2" id="add-record" onsubmit="return validateLiveNews('add-record');" action="{{url('admin/add-live-news')}}" method="POST" enctype="multipart/form-data"> @csrf 
        <div class="col-sm-12">
          <div class="mb-1">
            <label class="form-label" for="company_name">{{__('lang.admin_company_name')}} <span class="required">*</span></label>
            <input type="text" class="form-control dt-full-name" id="company_name" placeholder="{{__('lang.admin_company_name_placeholder')}}" name="company_name">
          </div>
        </div>
        <div class="col-sm-12">
          <div class="mb-1">
            <label class="form-label" for="url">{{__('lang.admin_url')}} <span class="required">*</span></label>
            <input type="text" class="form-control dt-full-name" id="url" placeholder="{{__('lang.admin_url_placeholder')}}" class="form-control" name="url">
          </div>
        </div>
        <div class="col-sm-12">
          <div class="mb-1">
