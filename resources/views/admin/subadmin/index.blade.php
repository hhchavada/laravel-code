...
<div class="container-xxl flex-grow-1 container-p-y">
  <h4 class="fw-bold py-3 mb-4 display-inline-block">
    <span class="text-muted fw-light">
      <a href="{{url('admin/dashboard')}}">{{__('lang.admin_dashboard')}}</a> / </span> {{__('lang.admin_subadmin')}} {{__('lang.admin_list')}}
  </h4>
  ...
  <button class="btn btn-secondary btn-primary float-right mt-3" type="button" data-bs-toggle="offcanvas" data-bs-target="#add-new-record" aria-controls="add-new-record">
    <span>
      <span class="d-md-inline-block d-none">{{__('lang.admin_create_subadmin')}}</span>
    </span>
  </button>
  @endcan
  <div class="card margin-bottom-20">
      ...
      <form method="get">
        <div class="row">
          ...
          <div class="col-sm-3 display-inline-block">
            ...
            <a type="reset" class="btn btn-outline-secondary" href="{{url('admin/sub-admin')}}">{{__('lang.admin_reset')}}</a>
          </div>
        </div>
      </form>
    </div>
    ...
    <div class="card">
        <div class="card-header">
          <h5 class="card-title display-inline-block">{{__('lang.admin_subadmin')}} {{__('lang.admin_list')}}</h5>
        ...
        <div class="table-responsive text-nowrap"> @include('admin/subadmin/table') </div>
    ...
    <div class="offcanvas offcanvas-end" id="add-new-record">
        <div class="offcanvas-header border-bottom">
            <h5 class="offcanvas-title" id="exampleModalLabel">{{__('lang.admin_add_subadmin')}}</h5>
            <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body flex-grow-1">
            <form class="add-new-record pt-0 row g-2" id="add-record" onsubmit="return validateSubadmin('add-record');" action="{{ url('admin/add-sub-admin') }}" method="POST" enctype="multipart/form-data">
                @csrf 
                <div class="col-sm-12">
                    <div class="mb-1">
                        <label class="form-label" for="name">{{ __('lang.admin_name') }} <span class="required">*</span></label>
                        <input type="text" class="form-control" id="name" placeholder="{{ __('lang.admin_name_placeholder') }}" name="name" required>
                    </div>
                </div>
                <div class="col-sm-12">
                    <div class="mb-1">
                        <label class="form-label" for="email">{{ __('lang.admin_email') }} <span class="required">*</span></label>
                        <input type="email" class="form-control" id="email" placeholder="{{ __('lang.admin_email_placeholder') }}" name="email" autocomplete="off" required>
                        <!-- Dummy Field -->
                ...
                <div class="col-sm-12">
                    <div class="mb-1">
                        <label class="form-label" for="password">{{ __('lang.admin_password') }} <span class="required">*</span></label>
                        <input type="password" class="form-control" id="password" placeholder="{{ __('lang.admin_password_placeholder') }}" name="password" autocomplete="new-password" required>
                        <!-- Dummy Field -->
                    ...
                    <div class="d-flex-p">
                      ...
                      <div class="img-footer">
                        <label class="btn btn-primary me-75 mb-0" for="change-picture">
                          ...
                          <input class="form-control" type="file" name="photo" id="change-picture" hidden accept="image/png, image/jpeg, image/jpg" name="image" onclick="showImagePreview('change-picture','image-preview',512,512);"/>
